...
class PayrollController extends Controller
{
            ...
            return Datatables::of($payrolls)
                ->addColumn(
                    ...
                    function ($row) {
                        ...
                        if ($row->payment_status == 'paid' && auth()->user()->can('essentials.create_payroll')) {
                            $html .= '<li><a href="' . action([\Modules\Essentials\Http\Controllers\PayrollController::class, 'sendPayrollEmail'], [$row->id]) . '" class=""><i class="fa fa-money"></i> ' . __('messages.send_email') . '</a></li>';
                        }
    ...
    public function create()
    {
        ...
        if (!empty($add_payroll_for)) {
            ...
            foreach ($employees as $employee) {
                ...
                $payrolls[$employee->id]['unit_salary'] = $employee->essentials_salary;
                ...
                //calculate the  working days salary
                $month_days = \Carbon::now()->month($month)->daysInMonth;
                $arr_salary_period = ['week' => 7, 'month' => $month_days, 'day' => 1];

                $pay_period = empty($employee->essentials_pay_period) ? $employee->essentials_pay_period : "month";
                ...
                if (!empty($employee->essentials_salary)) {
                    $essentials_salary = (float) $employee->essentials_salary / $arr_salary_period[$pay_period];
                    $essentials_salary * $total_days_worked;
                    $payrolls[$employee->id]['essentials_salary'] = round($essentials_salary * $total_days_worked);
                } else {
                    $payrolls[$employee->id]['essentials_salary'] = 0;
                }

                // $payrolls[$employee->id]['essentials_salary'] = !empty($employee->essentials_salary && $this->essentialsUtil->getTotalDaysWorkedForGivenDateOfAnEmployee($business_id, $employee->id, $start_date, $end_date)) ? round(($employee->essentials_salary / $pay_period) * $this->essentialsUtil->getTotalDaysWorkedForGivenDateOfAnEmployee($business_id, $employee->id, $start_date, $end_date)) :0;
                ...
                //Get Claim & Reimbursement
                $claim_and_reimbursment = $this->essentialsUtil->getEmployeeClaimAndReimbursement($business_id, $employee->id, $start_date, $end_date);

                foreach ($claim_and_reimbursment as $ad) {
                    ...
                    $payrolls[$employee->id]['allowances']['reimbursement_id'][] = $ad->id;
                }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($payrolls as $key => $payroll) {
                ...
                $payroll['essentials_unit_salary'] = $this->transactionUtil->num_uf($payroll['total']);
                ...
                if (!empty($payroll["reimbursement_ids"]) && $payroll_group->status == 'final') {
                    $ids = $payroll["reimbursement_ids"];
                    EssentialsClaimReimbursement::whereIn('id', $ids)->update(['is_approved' => '2']);
                }
                //$month = \Carbon::createFromFormat('m', $transaction['month'])->format('F');

                $transaction['pdf_name'] = 'Payroll-' . $transaction->ref_no . '.pdf';
                $transaction['pdf'] = $this->generatePayslip($transaction->id);
                $user = User::find($transaction['expense_for']);
                if ($notify_employee && $payroll_group->status == 'final') {

                    $transaction->action = 'created';
                    \Notification::route('mail', $user->email)
                        ->notify(new PayrollNotification($transaction));
    ...
    public function show($id)
    {
        ...
        if (!empty($payroll)) {
            $employee_id = $payroll->toArray()["expense_for"];
            $ytd_payroll = $this->essentialsUtil->getEmployeeeYTDPayroll($employee_id, $business_id, $start_of_month);
        }
        ...
        return view('essentials::payroll.show')
            ->with(compact('payroll', 'month_name', 'allowances', 'deductions', 'year', 'payment_types',
                ...
                'total_work_duration', 'location', 'total_days_present', 'ytd_payroll'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $payroll['pdf_name'] = 'Payroll-' . $payroll->ref_no . '.pdf';
            $payroll['pdf'] = $this->generatePayslip($payroll->id);

            $user = User::find($payroll['expense_for']);

            \Notification::route('mail', $user->email)
                ->notify(new PayrollNotification($payroll));
    ...
    public function getEditPayrollGroup($id)
    {
        ...
        foreach ($payroll_group->payrollGroupTransactions as $transaction) {
            ...
            $payrolls[$transaction->expense_for]['essentials_salary'] = $transaction->essentials_unit_salary;
        ...
        try {
            ...
            foreach ($payrolls as $key => $payroll) {
                ...
                if (!empty($payroll_trans)) {
                    ...
                    if ($notify_employee && $payroll_group->status == 'final') {
                        $payroll_trans['pdf_name'] = 'Payroll-' . $payroll_trans->ref_no . '.pdf';
                        $payroll_trans['pdf'] = $this->generatePayslip($payroll_trans->id);
    ...
    public function addPayment($id)
    {
        ...
        $payroll_group = PayrollGroup::where('business_id', $business_id)
            ->with(['payrollGroupTransactions', 'payrollGroupTransactions.transaction_for', 'payrollGroupTransactions.payment_lines', 'businessLocation', 'business'])
            ->findOrFail($id);
        ...
        foreach ($payroll_group->payrollGroupTransactions as $transaction) {
            ...
            {
                $ref_nos[] = $payment_lines->payment_lines;
            }
            $payrolls[$transaction->expense_for]['payment_ref_no'] = implode(',', $ref_nos);
            $payrolls[$transaction->expense_for]['bank_details'] = json_decode($transaction->transaction_for->bank_details, true);
        }
    ...
    public function generatePayslip($id)
    {
        ...
        // return view('essentials::payroll.slip')
        //     ->with(compact('payroll', 'month_name', 'allowances', 'deductions', 'year', 'payment_types',
        //         'bank_details', 'designation', 'department', 'final_total_in_words', 'total_leaves', 'days_in_a_month',
        //         'total_work_duration', 'location', 'total_days_present', 'ytd_payroll'));

        //Generate pdf
        $body = view('essentials::payroll.slip')
            ->with(compact('payroll', 'month_name', 'allowances', 'deductions', 'year', 'payment_types',
                'bank_details', 'designation', 'department', 'final_total_in_words', 'total_leaves', 'days_in_a_month',
                'total_work_duration', 'location', 'total_days_present', 'ytd_payroll'));
        ...
        $mpdf->SetWatermarkText(request()->session()->get('business.name'), 0.1);
        $mpdf->showWatermarkText = true;
        $mpdf->SetTitle('Payroll-' . $payroll->ref_no . '.pdf');
        $mpdf->WriteHTML($body);
    ...
    public function downloadPayslip($id)
    {
        ...
        $pdf = Pdf::loadView('essentials::payroll.slip', compact('payroll', 'month_name', 'allowances', 'deductions', 'year', 'payment_types',
            'bank_details', 'designation', 'department', 'final_total_in_words', 'total_leaves', 'days_in_a_month',
            'total_work_duration', 'location', 'total_days_present', 'ytd_payroll'));
        return $pdf->download('invoice.pdf');
    }

    public function sendPayrollEmail($id)
    {
        $business_id = request()->session()->get('user.business_id');
