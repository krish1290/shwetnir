...
class EssentialsLeaveController extends Controller
{
    ...
    public function index()
    {
        ...
        // dd($can_crud_all_leave,$can_crud_own_leave);
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($leaves)
                ...
                ->editColumn('status', function ($row) {
                    ...
                    $user_id = EssentialsLeave::where('id', $row->id)->first();
                    if(!empty($user_id)){
                      $managerUser = User::where('id', $user_id->user_id)->first();
                      $managerUser_id = $managerUser->parent_id;

                      $user_can_manage_leave = auth()->user()->can('essentials.crud_all_leave') || auth()->user()->can('essentials.approve_leave');
    ...
    public function create()
    {
        ...
        $leave_types = EssentialsLeaveType::forDropdownnew($business_id);
    ...
    public function store(Request $request)
    {
      // dd($request->all());
        ...
        $diff = $start_date->diffInDays($end_date);
        $diff += 1;
        $leave_type = EssentialsLeaveType::FindOrFail($request->input('essentials_leave_type_id'));
        $check_leave_balance = EssentialsUserLeaveAndDeduction::where('leave_id',$request->input('essentials_leave_type_id'))->where('user_id',request()->session()->get('user.id'))->first();

        if($check_leave_balance->balance<$diff  && $leave_type->type ==0){

            $output = ['success' => false,
                'msg' => __('messages.leave_balance_check'),
            ];
             return $output;
        }
        ...
        try {
            ...
            if($leave_type->type ==0){
             $check_leave_balance->balance = $check_leave_balance->balance-$diff;
             $check_leave_balance->save();
            }
      ...
      // dd($request->all());
        ...
        try {
            ...
            $leave = EssentialsLeave::where('business_id', $business_id)
                ->find($input['leave_id']);
            // if (!in_array($leave->user_id, $managerUsers) && !(auth()->user()->can('superadmin'))) {
            //   // dd('dkd');
            //     abort(403, 'Unauthorized action.');
            // }
            ...
            $checkLeaveType = EssentialsLeaveType::where('id',$leave->essentials_leave_type_id)->first();
            if ($input['status'] == 'cancelled' && !$checkLeaveType == 'Unpaid Leave') {
              $start_date = \Carbon::parse($leave->start_date);
              $end_date = \Carbon::parse($leave->end_date);

              $diff = $start_date->diffInDays($end_date);
              $diff += 1;
              $CheckBalance = EssentialsUserLeaveAndDeduction::where('user_id',$leave->user_id)->where('leave_id',$leave->essentials_leave_type_id)->first();
              // dd($CheckBalance);
              $CheckBalance->balance = $CheckBalance->balance+$diff;
              $CheckBalance->save();

              $deleteTransactionLeaveData = EssentialsUserLeaveAndDeductionsTransaction::where('user_id',$leave->user_id)->where('leave_type_id',$leave->essentials_leave_type_id)->delete();
            }
            ...
            $leave->notify(new LeaveStatusNotification($leave));
    ...
    public function getUserLeaveSummary()
    {
        ...
        $userHaveLeaves = array_keys(EssentialsLeaveType::forDropdownnew($business_id));
        $leave_types = EssentialsLeaveType::where('business_id', $business_id)->whereIn('id',$userHaveLeaves)
            ->get();
            // dd($leave_types);
