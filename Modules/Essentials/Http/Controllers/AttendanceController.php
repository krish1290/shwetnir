...
class AttendanceController extends Controller
{
    ...
    public function index()
    {
        ...
        $view_all_attendance = auth()->user()->can('essentials.view_all_attendance');
        $can_view_own_attendance = auth()->user()->can('essentials.view_own_attendance');
        if (!$view_all_attendance && !$can_view_own_attendance) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if (request()->ajax()) {
            ...
            // dd($view_all_attendance,$can_view_own_attendance);
            if (!$view_all_attendance && $can_view_own_attendance) {
            ...
            return Datatables::of($attendance)
                ->addColumn(
                    'action',
                    '@can("essentials.edit_attendance") <button data-href="{{action(\'\Modules\Essentials\Http\Controllers\AttendanceController@edit\', [$id])}}" class="btn btn-xs btn-primary btn-modal" data-container="#edit_attendance_modal"><i class="glyphicon glyphicon-edit"></i> @lang("messages.edit")</button>&nbsp;@endcan
                        @can("essentials.delete_attendance")<button class="btn btn-xs btn-danger delete-attendance" data-href="{{action(\'\Modules\Essentials\Http\Controllers\AttendanceController@destroy\', [$id])}}"><i class="fa fa-trash"></i> @lang("messages.delete")</button> @endcan
                        '
                )
                ->addColumn('checkbox', function ($row) {
                  $check_box =  '<input type="checkbox" class="sub_chk" data-id="'.$row->id.'">';
                  return $check_box;
                })
                ...
                ->rawColumns(['action', 'clock_in', 'work_duration', 'clock_out','checkbox'])
    ...
    {
      $ids = $request->ids;
      EssentialsAttendance::whereIn('id',explode(",",$ids))->delete();
            ...
            if (empty($attendance_by_shift[$data->essentials_shift_id])) {
                ...
                foreach ($data->shift->user_shifts as $user_shift) {
                    if (!empty($user_shift->start_date) && !empty($user_shift->end_date) && $date_obj->between(\Carbon::parse($user_shift->start_date), \Carbon::parse($user_shift->end_date))) {
                        $total_users++;
                        $all_users[] = !empty($user_shift->user->user_full_name) ? $user_shift->user->user_full_name : '';
                    }
                }
                $attendance_by_shift[$data->essentials_shift_id] = [
                    'present' => 1,
                    'shift' => $data->shift->user_shifts['name'],
                    'total' => $total_users,
        ...
        try {
            ...
            if ($request->hasFile('attendance')) {
                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    //clockin time
                    if (!empty($value[1])) {
                        $temp['clock_in_time'] = \Carbon::instance(\PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject(trim($value[1])));
                    } else {
                        $is_valid = false;
                        $error_msg = "Clock in time is required in row no. $row_no";
                        break;
                    }
                    $clok_out_time = !empty($value[2]) ? trim($value[2]) : null;
                    $temp['clock_out_time'] = \Carbon::instance(\PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject(trim($clok_out_time)));
    ...
    public function getAttendanceRow($user_id)
    {
        ...
        $shifts = Shift::join('essentials_user_shifts as eus', 'eus.essentials_shift_id', '=', 'essentials_shifts.id')
            ...
            ->where('eus.start_date', '<=', \Carbon::now()->format('Y-m-d'))->get()->toArray();
            // dd($shifts);
