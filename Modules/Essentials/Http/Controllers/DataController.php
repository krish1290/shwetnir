...
class DataController extends Controller
{
    ...
    public function user_permissions()
    {
        return [
            ...
            [
                'value' => 'essentials.view_all_attendance',
                'label' => __('essentials::lang.view_all_attendance'),
                'default' => false,
                'is_radio' => true,
                'radio_input_name' => 'view_all_attendance',
            ],
            [
                'value' => 'essentials.add_attendance',
                'label' => __('essentials::lang.add_attendance'),
                'default' => false,
            ],
            [
                'value' => 'essentials.edit_attendance',
                'label' => __('essentials::lang.edit_attendance'),
                'default' => false,
            ],
            [
                'value' => 'essentials.delete_attendance',
                'label' => __('essentials::lang.delete_attendance'),
                'default' => false,
            ],
            // [
            ...
            [
                'value' => 'essentials.approve_allowance_and_deduction',
                'label' => __('essentials::lang.approve_pay_component'),
                'default' => false,
            ],
            [
                'value' => 'essentials.view_claim_reimbursement',
                'label' => __('essentials::lang.view_claim_reimbursement'),
                'default' => false,
            ],
            [
                'value' => 'essentials.add_claim_reimbursement',
                'label' => __('essentials::lang.add_claim_reimbursementt'),
                'default' => false,
            ],
            [
                'value' => 'essentials.approve_claim_reimbursement',
                'label' => __('essentials::lang.approve_claim_reimbursement'),
                'default' => false,
            ],
            [
            ...
            [
                'value' => 'essentials.add_holiday',
                'label' => __('essentials::lang.add_holiday'),
                'default' => false,
            ],
            [
                'value' => 'essentials.delete_holiday',
                'label' => __('essentials::lang.delete_holiday'),
                'default' => false,
            ],
            [
                'value' => 'essentials.edit_holiday',
                'label' => __('essentials::lang.edit_holiday'),
                'default' => false,
            ],
        ];
    }
        ...
        if ($is_essentials_enabled) {
            Menu::modify('admin-sidebar-menu', function ($menu) {
                $menu->url(
                    ...
                    ['icon' => 'la las la-users', 'has_sub_item' => 'no', 'active' => request()->segment(1) == 'hrm', 'style' => config('app.env') == 'demo' ? 'background-color: #605ca8 !important;' : '']
                )
                    ->order(87);
                ...
                //     ['icon' => 'la las la-check-circle', 'has_sub_item' => 'no', 'active' => request()->segment(1) == 'essentials', 'style' => config('app.env') == 'demo' ? 'background-color: #001f3f !important;' : '']
                // )
                //     ->order(87);
                // $menu->url(
                //     action([\Modules\Essentials\Http\Controllers\ToDoController::class, 'index']),
                //     __('essentials::lang.essentials'),
                //     ['icon' => 'fa fas fa-check-circle', 'has_sub_item' => 'no', 'active' => request()->segment(1) == 'essentials', 'style' => config('app.env') == 'demo' ? 'background-color: #001f3f !important;' : '']
                // )
                //     ->order(87);
                if (auth()->user()->can('essentials.view_claim_reimbursement')) {
                    $menu->url(
                        action([\Modules\Essentials\Http\Controllers\ClaimReimbursementController::class, 'index']),
                        __('essentials::lang.claim_reimbursement'),
                        ['icon' => 'las la-money-bill-wave', 'has_sub_item' => 'no', 'active' => request()->segment(1) == 'essentials', 'style' => config('app.env') == 'demo' ? 'background-color: #001f3f !important;' : '']
                    )
                        ->order(88);
                }
                if (auth()->user()->can('training.view_training')) {
                    $menu->url(
                        action([\Modules\Essentials\Http\Controllers\TrainingController::class, 'index']),
                        __('essentials::lang.training'),
                        ['icon' => 'las la-chalkboard-teacher', 'has_sub_item' => 'no', 'active' => request()->segment(1) == 'essentials', 'style' => config('app.env') == 'demo' ? 'background-color: #001f3f !important;' : '']
                    )
                        ->order(88);
                }
            });
    ...
    public function moduleViewPartials($data)
    {
        if ($data['view'] == 'manage_user.create' || $data['view'] == 'manage_user.edit') {
            ...
            $leave_type = EssentialsLeaveType::forDropdown($business_id);
            ...
            $leave_type_ids = [];
            if (!empty($user)) {
                $leave_type_ids = EssentialsUserLeaveAndDeduction::where('user_id', $user->id)
                    ->pluck('leave_id')
                    ->toArray();
            }
            ...
            return view('essentials::partials.user_form_part', compact('departments', 'designations', 'user', 'pay_comoponenets','leave_type','leave_type_ids', 'allowance_deduction_ids', 'locations'))
                ->render();
    ...
    public function afterModelSaved($data)
    {
        if ($data['event'] = 'user_saved') {
            ...
            $leave_types = EssentialsLeaveType::where('business_id',$user->business_id)->get();

            // foreach ($leave_types as  $leave_type) {
            //        $euld = EssentialsUserLeaveAndDeduction::where('user_id',$user->id)->where('leave_id',$leave_type->id)->first();
            //        if(empty($euld->leave_id)){
            //           EssentialsUserLeaveAndDeduction::insert(['user_id' => $user->id, 'leave_id' => $leave_type->id]);
            //        }
            //
            //  }
            ...
            //EssentialsUserLeaveAndDeduction::where('user_id', $user->id)
                //->whereNotIn('allowance_deduction_id', $non_deleteable_pc_ids)
                //->delete();

            /*if (!empty(request()->input('leave_type'))) {
                $leave_type = request()->input('leave_type');
                foreach ($leave_type as $key => $leave_tp) {
                    EssentialsUserLeaveAndDeduction::insert(['user_id' => $user->id, 'leave_id' => $leave_tp]);
                }
            } */
        }
    }
