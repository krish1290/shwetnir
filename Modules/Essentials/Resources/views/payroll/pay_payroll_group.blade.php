...
@section('content')
    ...
    <section class="content">
        <div class="row">
            {!! Form::open([
                'url' => action([\Modules\Essentials\Http\Controllers\PayrollController::class, 'postAddPayment']),
                'method' => 'post',
                'id' => 'payroll_group_payment',
            ]) !!}
                    ...
                    <div class="box-body">
                        ...
                        <div class="row margin-bottom-20">
                            ...
                            <div class="col-md-6 text-center">
                                ...
                                <b class="font-17">
                                    @lang('lang_v1.cn_no_label'):
                                </b>
                                {{ $payroll_group->ref_no }}
                            </div>
                        </div>
                        <div class="table-responsive mt-15">
                            <table class="table" id="payroll-group-table" style="width: 100% !important;">
                                <tr>
                                    ...
                                    <th>@lang('lang_v1.cn_no_label')</th>
                                    <th>
                                ...
                                @foreach ($payrolls as $id => $payroll)
                                    <tr data-id="{{ $id }}">
                                        <input type="hidden" name="payments[{{ $id }}][transaction_id]"
                                            value="{{ $payroll['transaction_id'] }}">
                                        <input type="hidden" name="payments[{{ $id }}][employee_id]"
                                            value="{{ $payroll['employee_id'] }}">
                                        <input type="hidden" name="payments[{{ $id }}][final_total]"
                                            value="{{ $payroll['final_total'] }}">
                                        <td>
                                            {{ $payroll['employee'] }}
                                        </td>
                                        <td>
    ...
    <script type="text/javascript">
        $(function() {
            ...
            $('select.payment_types').on('change', function() {
                ...
                if (method == 'card') {
                    $('#card_' + id).show();
                    $(`#custom_pay_1_${id}, #custom_pay_2_${id}, #custom_pay_3_${id}, #cheque_${id}, #bank_transfer_${id}`)
                        .hide();
                } else if (method == 'cheque') {
                    $('#cheque_' + id).show();
                    $(`#custom_pay_1_${id}, #custom_pay_2_${id}, #custom_pay_3_${id}, #bank_transfer_${id}, #card_${id}`)
                        .hide();
                } else if (method == 'bank_transfer') {
                    $('#bank_transfer_' + id).show();
                    $(`#custom_pay_1_${id}, #custom_pay_2_${id}, #custom_pay_3_${id}, #cheque_${id}, #card_${id}`)
                        .hide();
                } else if (method == 'custom_pay_1') {
                    $('#custom_pay_1_' + id).show();
                    $(`#custom_pay_2_${id}, #custom_pay_3_${id}, #cheque_${id}, #bank_transfer_${id}, #card_${id}`)
                        .hide();
                } else if (method == 'custom_pay_2') {
                    $('#custom_pay_2_' + id).show();
                    $(`#custom_pay_1_${id}, #custom_pay_3_${id}, #cheque_${id}, #bank_transfer_${id}, #card_${id}`)
                        .hide();
                } else if (method == 'custom_pay_3') {
                    $('#custom_pay_3_' + id).show();
                    $(`#custom_pay_1_${id}, #custom_pay_2_${id}, #cheque_${id}, #bank_transfer_${id}, #card_${id}`)
                        .hide();
                } else {
                    $(`#custom_pay_1_${id}, #custom_pay_2_${id}, #custom_pay_3_${id}, #cheque_${id}, #bank_transfer_${id}, #card_${id}`)
                        .hide();
                }
            });
        });
