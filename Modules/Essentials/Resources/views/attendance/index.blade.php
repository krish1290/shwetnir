...
@section('content')
                    ...
                    <div class="page-header">
                        <nav class="breadcrumb-one" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page"><span>@lang('essentials::lang.attendance')</span>
                                </li>
        ...
        @if ($is_employee_allowed)
            <div class="row">
                <div class="col-md-12 text-center">
                    ...
                    @if (!empty($clock_in))
                        <br>
                        <small class="text-muted">@lang('essentials::lang.clocked_in_at'):
                            {{ @format_datetime($clock_in->clock_in_time) }}</small>
                    @endif
                </div>
            </div>
        @endif
            ...
            <div class="col-md-12">
                <div class="nav-tabs-custom">
                    <ul class="nav nav-tabs">
                        @can('essentials.view_all_attendance')
                            <li class="active">
                        ...
                        @endcan
                        <li @if (!auth()->user()->can('essentials.view_all_attendance')) class="active" @endif>
                        ...
                        @can('essentials.view_all_attendance')
                            <li>
                                <a href="#attendance_by_shift_tab" data-toggle="tab" aria-expanded="true"><i
                                        class="fas fa-user-check" aria-hidden="true"></i> @lang('essentials::lang.attendance_by_shift')</a>
                            </li>
                            <li>
                                <a href="#attendance_by_date_tab" data-toggle="tab" aria-expanded="true"><i
                                        class="fas fa-calendar" aria-hidden="true"></i> @lang('essentials::lang.attendance_by_date')</a>
                            </li>
                            <li>
                                <a href="#import_attendance_tab" data-toggle="tab" aria-expanded="true"><i
                                        class="fas fa-download" aria-hidden="true"></i> @lang('essentials::lang.import_attendance')</a>
                            </li>
                        @endcan
                    </ul>
                    <div class="tab-content">
                        ...
                        @endcan
                        <div class="tab-pane @if (!auth()->user()->can('essentials.view_all_attendance')) active @endif" id="attendance_tab">
                            <div class="row">
                                @can('essentials.view_all_attendance')
                                    <div class="col-md-3">
                                ...
                                @can('essentials.view_all_attendance')
                                    <div class="col-md-3 spacer">
                                        <button type="button" class="btn btn-primary btn-modal pull-right delete_all" data-url="{{ url('hrm/deleteSelected') }}">
                                            <i class="fa fa-trash"></i>
                                            @lang('essentials::lang.delete_selected_attandance')
                                        </button>
                                    </div>
                                @endcan
                            </div>
    ...
    <div class="modal fade" id="user_shift_modal" tabindex="-1" role="dialog" aria-labelledby="gridSystemModalLabel">
    </div>
    <div class="modal fade" id="edit_shift_modal" tabindex="-1" role="dialog" aria-labelledby="gridSystemModalLabel">
    </div>
    <div class="modal fade" id="shift_modal" tabindex="-1" role="dialog" aria-labelledby="gridSystemModalLabel">
...
$(document).on('click','#master', function (e) {
    // Use prop('checked') to get the current state
...
$('.delete_all').on('click', function (e) {
    ...
    if (allVals.length <= 0) {
        ...
        var check = confirm("Are you sure you want to delete selected attendance?");
        if (check == true) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data['success']) {
                        ...
                        window.LaravelDataTables["#attendance_table"].ajax.reload();
                    } else if (data['error']) {
            ...
            // No need to loop through allVals again
            // $('table tr').filter("[data-row-id='" + allVals[0] + "']").remove();
        }
    }
});
...
// Rest of your code...
    ...
    <script type="text/javascript">
        $(document).ready(function() {
          ...
          $('#date_range').on('cancel.daterangepicker', function(ev, picker) {
            ...
            attendance_table = $('#attendance_table').DataTable({
                ...
                ajax: {
                    ...
                    "data": function(d) {
                        ...
                        if ($('#date_range').val()) {
                            var start = $('#date_range').data('daterangepicker').startDate.format(
                                'YYYY-MM-DD');
                            var end = $('#date_range').data('daterangepicker').endDate.format(
                                'YYYY-MM-DD');
                            d.start_date = start;
                            d.end_date = end;
                        }
            ...
            $('#shift_modal, #edit_shift_modal').on('shown.bs.modal', function(e) {
                ...
                $('#shift_modal #start_time, #shift_modal #end_time, #edit_shift_modal #start_time, #edit_shift_modal #end_time')
                    .datetimepicker({
            ...
            @can('essentials.view_all_attendance')
                ...
                $('#attendance_by_date_filter').daterangepicker(
                    dateRangeSettings,
                    function(start, end) {
                        $('#attendance_by_date_filter').val(start.format(moment_date_format) + ' ~ ' + end
                            .format(moment_date_format));
                    }
                );
        ...
        function get_attendance_summary() {
            ...
            $.ajax({
                url: '{{ action([\Modules\Essentials\Http\Controllers\AttendanceController::class, 'getUserAttendanceSummary']) }}?user_id=' +
        ...
        $('#attendance_modal').on('dp.change', '#clock_in_time', function() {
            if ($('#clock_out_time').data("DateTimePicker")) {
                $('#clock_out_time').data("DateTimePicker").options({
                    minDate: $(this).data("DateTimePicker").date()
                });
        ...
        function get_attendance_by_shift() {
            data = {
                date: $('#attendance_by_shift_date_filter').val()
            };
            $.ajax({
        ...
        $(document).on('change', '.emp_shift', function(e) {
            var user_id = $(this).find(':selected').attr('data-user_id');
            var start_time = $(this).find(':selected').attr('data-start_time');
            var end_time = $(this).find(':selected').attr('data-end_time');
            var current_date = moment().format(moment_date_format);
            var shift_start_time = current_date + ' ' + start_time;
            var shift_end_time = current_date + ' ' + end_time;
            $('#shift_clock_in_time-' + user_id).data("DateTimePicker").clear();
            $('#shift_clock_in_time-' + user_id).data("DateTimePicker").date(moment(shift_start_time).format(
                moment_date_format + ' ' + moment_time_format));

            $('#shift_clock_out_time-' + user_id).data("DateTimePicker").clear();
            $('#shift_clock_out_time-' + user_id).data("DateTimePicker").date(moment(shift_end_time).format(
                moment_date_format + ' ' + moment_time_format));
