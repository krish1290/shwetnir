...
.navbar-nav li {
    margin-right: 20px; /* Spacing between the list items */
}
...
.navbar-nav li.active a {
    ...
    background-color: #007bff; /* Background for active link */
...
<section class="no-print">
    <nav class="tw-transition-all tw-duration-5000 tw-shrink-0 tw-rounded-2xl tw-m-[16px] tw-border-2 !tw-bg-white">
        <div class="container-fluid">
            ...
            <ul class="nav navbar-nav">
                @can('manufacturing.access_recipe')
                    <li class="@if(request()->segment(1) == 'manufacturing' && in_array(request()->segment(2), ['recipe', 'add-ingredient'])) active @endif">
                ...
                @can('manufacturing.access_production')
                    <li class="@if(request()->segment(2) == 'production') active @endif">
                        <a href="{{action([\Modules\Manufacturing\Http\Controllers\ProductionController::class, 'index'])}}">
                            @lang('manufacturing::lang.production')
                        </a>
                    </li>

                    <li class="@if(request()->segment(1) == 'manufacturing' && request()->segment(2) == 'settings') active @endif">
