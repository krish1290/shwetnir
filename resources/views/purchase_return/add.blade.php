...
<section class="content">
	{!! Form::open(['url' => action([\App\Http\Controllers\PurchaseReturnController::class, 'savereturn']), 'method' => 'post', 'id' => 'purchase_return_form' ]) !!}
	{!! Form::hidden('transaction_id', request()->id); !!}
	...
	@component('components.widget', ['class' => 'box-primary'])
			            ...
			            <tr>
			              	...
			              	<td>
					            ...
					            @if($purchase_line->product->enable_stock) 
			              			data-rule-max-value="{{array_key_exists($purchase_line->product_id.'_'.$purchase_line->variation_id, $can_return_qty) ? $can_return_qty[$purchase_line->product_id.'_'.$purchase_line->variation_id] : $purchase_line->quantity}}"
			              			data-msg-max-value="@lang('validation.custom-messages.quantity_not_available', ['qty' => $purchase_line->quantity, 'unit' => $unit_name ])" 
			              		@endif
					            >
					            <input type="hidden" class="unit_price" value="{{@num_format($purchase_line->purchase_price_inc_tax)}}">
					            <input name="products[{{$loop->index}}][unit_price_inc_tax]" type="hidden" class="unit_price" value="{{@num_format($purchase_line->unit_price_inc_tax)}}">
								<input type="hidden" name="products[{{$loop->index}}][product_id]" class="form-control product_id" value="{{$purchase_line->product_id}}">
								<input name="products[{{$loop->index}}][sell_line_id]" type="hidden" value="{{$purchase_line->parent_sell_line_id}}">
								<input name="products[{{$loop->index}}][sell_line_return_id]" type="hidden" value="{{$purchase_line->id}}">
								<input name="products[{{$loop->index}}][unit_price]" type="hidden" value="{{$purchase_line->unit_price_inc_tax}}">
								<input name="products[{{$loop->index}}][price]" type="hidden" value="{{$purchase_line->formatted_qty * $purchase_line->unit_price_inc_tax}}">
								<input name="reloadForm" class="reloadForm" type="hidden" value="1">
			              	</td>
			              	<td>
		...
		<div class="row">
			<div class="col-sm-12 text-right">
				...
				<input type="hidden" name="final_total" value="0" id="net_return_field">
			</div>
		</div>
		<br>
		<div class="row">
...
<script type="text/javascript">
	...
	function update_purchase_return_total(){
		...
		$('input#net_return_field').val(net_return_inc_tax);
	}
</script>
@endsection
