...
<div class="row">
	<div class="pos-form-actions">
		<div class="col-md-12">
			...
			<!-- <button type="button" class="@if($is_mobile) col-xs-6 @endif btn bg-info text-white btn-default btn-flat" id="pos-create" @if(!empty($only_payment)) disabled @endif><i class="fas fa-edit"></i> @lang('sale.create')</button> -->
			<!-- <button type="button" class="@if($is_mobile) col-xs-6 @endif btn bg-info text-white btn-default btn-flat @if($pos_settings['disable_draft'] != 0) hide @endif" id="pos-received-order" @if(!empty($only_payment)) disabled @endif><i class="fas fa-edit"></i> @lang('sale.draft')</button> -->
			@if(strpos($url, 'create') == false && strpos($url, 'edit') == false)
			<button type="submit" class="@if($is_mobile) col-xs-6 @endif btn bg-red text-white btn-default btn-flat @if($pos_settings['disable_duplicate'] != 0) hide @endif" id="pos-duplicate" @if(!empty($only_payment)) disabled @endif><i class="fas fa-edit"></i> @lang('sale.duplicate')</button>
			@endif
			...
			<!-- <button type="button" class="btn btn-default bg-yellow btn-flat @if($is_mobile) col-xs-6 @endif" id="pos-received-order" @if(!empty($only_payment)) disabled @endif><i class="fas fa-edit"></i> @lang('lang_v1.received_order')</button> -->
			...
			<button type="button" class="btn btn-success" id="pos-create-finalize" data-pay_method="cash" title="@lang('tooltip.express_checkout')"> <i class="fas fa-money-bill-alt" aria-hidden="true"></i> @lang('lang_v1.create')</button>

			@if(empty($edit))
				<button type="button" class=" btn btn-danger text-white btn-default btn-flat" id="pos-cancel"> <i class="fas fa-window-close"></i> @lang('sale.cancel')</button>
			@else
			...
			@if(!$is_mobile)
			<div class="btn bg-navy pos-total text-white" style="padding:0px 10px;">
