...
<section class="content no-print">
	...
	{!! Form::open(['url' => action([\App\Http\Controllers\SellReturnController::class, 'savereturn']), 'method' => 'post', 'id' => 'sell_return_form' ]) !!}
						...
						<tbody>
							...
							<tr>
								...
								<td>
									...
									data-rule-max-value="{{array_key_exists($sell_line->product_id.'_'.$sell_line->variation_id, $can_return_qty) ? $can_return_qty[$sell_line->product_id.'_'.$sell_line->variation_id] : $sell_line->quantity}}" 
									...
									<input type="hidden" name="products[{{$loop->index}}][product_id]" class="form-control product_id" value="{{$sell_line->product_id}}">
									<input name="products[{{$loop->index}}][sell_line_id]" type="hidden" value="{{$sell_line->parent_sell_line_id}}">
									<input name="products[{{$loop->index}}][sell_line_return_id]" type="hidden" value="{{$sell_line->id}}">
									<input name="products[{{$loop->index}}][unit_price]" type="hidden" value="{{$sell_line->unit_price_inc_tax}}">
									<input name="products[{{$loop->index}}][price]" type="hidden" value="{{$sell_line->formatted_qty * $sell_line->unit_price_inc_tax}}">
