$(document).ready(function () {
    ...
    $('body').on('click', '.btn-group', function (e) {
        $('.btn-group').on('click', 'shown.bs.dropdown', function () {
            $(".dataTables_scrollBody").css("overflow", "visible");
        });
        $('.btn-group').on('click', 'hidden.bs.dropdown', function () {
    ...
    //Start: CRUD for wastage types
    //Wastage Type table
    var wastage_types_table = $('#wastage_types_table').DataTable({
        dom: '<"row"<"col-md-6"B><"col-md-6"f> ><""rt> <"col-md-12"<"row"<"col-md-5"i><"col-md-7"p>>>',
        buttons: {
            buttons: [{
                extend: 'copy',
    ...
    $(document).on('submit', 'form#wastage_type_add_form', function (e) {
        e.preventDefault();
        var form = $(this);
        var data = form.serialize();
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.success == true) {
                    $('div.wastage_type_modal').modal('hide');
                    toastr.success(result.msg);
                    wastage_types_table.ajax.reload();
                } else {
                    toastr.error(result.msg);
                }
            },
    ...
    $(document).on('click', 'button.edit_wastage_type_button', function () {
        $('div.wastage_type_modal').load($(this).data('href'), function () {
            $(this).modal('show');

            $('form#wastage_type_edit_form').submit(function (e) {
                e.preventDefault();
                var form = $(this);
                var data = form.serialize();
    ...
    $(document).on('click', 'button.delete_wastage_type_button', function () {
        swal({
            title: LANG.sure,
            text: LANG.confirm_delete_wastage_type,
            icon: 'warning',
            buttons: true,
            dangerMode: true,
        }).then(willDelete => {
    ...
    $(document).on('ifUnchecked', '.input-icheck:not(.check_all)', function () {
        $(this).parents('.col-md-9').parent('div')
            .find('.col-md-2')
            .find('.input-icheck')
            .each(function() {
                $(this).removeAttr('checked');
                $(this).closest('.icheckbox_square-blue').removeClass('checked');
            });
    });
