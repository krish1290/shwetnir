$(document).ready(function () {
    ...
    }else if ($('form#duplicate_pos_sell_form').length > 0) {
      pos_total_row();
      pos_form_obj = $('form#duplicate_pos_sell_form');
    }else {
    ...
    //Save invoice as duplicate
    $('button#pos-duplicate').click(function () {
      // alert('zdklfk')
        //Check if product is present or not.
        if ($('table#pos_table tbody').find('.product_row').length <= 0) {
            toastr.warning(LANG.no_products_added);
    ...
    window.onbeforeunload = function () {
        if ($('form#duplicate_pos_sell_form').length == 0) {
            if ($('table#pos_table tbody tr').length > 0) {
                return LANG.sure;
            } else {
                return null;
...
function calculate_billing_details(price_total) {
    ...
    if ($('form#duplicate_pos_sell_form').length == 0 && $('form#edit_sell_form').length == 0) {
        __write_number($('.payment-amount').first(), total_payable_rounded);
    }
