$(document).ready(function () {
    ...
    $('#supplier_id').change(function () {
        ...
        $.ajax({
            method: 'POST',
            url: '/purchase-return/get_supplier_purchase',
            data: { supplier_id: supplier_id, location_id: location_id },
            dataType: 'json',
            success: function (result) {
                $('#purchase_reference').html('<option value="">Please Select</option>');
                $.each(result.purchases, function (key, value) {
                    $("#purchase_reference").append('<option value="' + key + '">' + value + '</option>');
                });
            },
        });
    });
    // Get product against purchase ref
    $('#purchase_reference').change(function () {
        ...
        $.ajax({
            method: 'POST',
            url: '/get_purchased_product',
            data: { transaction_id: transaction_id },
            dataType: 'json',
            success: function (result) {
                $('table#purchase_return_product_table tbody').html('');
                var i = 0;
                $.each(result, function (key, value) {
                    purchase_return_product_row(key, value, i);
                    i++;
                });
            },
        });
    ...
    $(document).on('click', 'button#submit_purchase_return_form', function (e) {
        ...
        if ($('form#purchase_return_form').valid()) {
            var final_total = parseFloat($('#total_return').text());
            $('form#purchase_return_form').append('<input type="hidden" value="'+final_total+'" name="final_total"');
...
function purchase_return_product_row(variation_id, transaction_id = 0, row_number = 0) {
