...
class Util
{
    ...
    public function orderApproveStatuses()
    {
        return ['0' => __('lang_v1.pending'), '1' => __('lang_v1.approved')];
    }
    ...
    public function payment_types($location = null, $show_advance = false, $business_id = null)
    {
        ...
        $payment_types = ['cash' => __('lang_v1.cash'),'create' => __('lang_v1.create'), 'card' => __('lang_v1.card'), 'cheque' => __('lang_v1.cheque'), 'bank_transfer' => __('lang_v1.bank_transfer'), 'other' => __('lang_v1.other')];
    ...
    private function sendSmsViaTwilio($data)
    {
        ...
        try {
            ...
            foreach ($numbers as $number) {
                // print_r($sms_settings);exit;
                
                $twilio->message($number, $data['sms_body']);
            }

            return true; // Indicate that the SMS sending was successful
        } catch (\Exception $e) {
            // Log or handle the exception as needed
            // For example, log the error message:
            error_log('Twilio SMS sending failed: ' . $e->getMessage());
    ...
    public function createUser($request)
    {
        $user_details = $request->only([
            'surname', 'first_name', 'last_name', 'email', 'parent_id',
    ...
    public function getRecursiveChildren($id, $items): array
    {
        $kids = [];
        foreach ($items as $key => $item) {
            if ($item['parent_id'] === $id) {
                ...
                if ($id !== $key) {
                    array_push($kids, ...$this->getRecursiveChildren($key, $items));
                }
            }
        }
        return $kids;
    ...
    public function getManagerUSers($business_id, $user_id)
    {

        $users_array = User::where('business_id', $business_id)->select('parent_id', 'id')->get();
        $items = [];
        foreach ($users_array as $user) {
            $items[$user->id] = $user->toArray();
        }
        $users = $this->getRecursiveChildren($user_id, $items);
        // dd($users);
        return $users;
    }
}
