...
class NotificationUtil extends Util
{
    ...
    public function autoSendNotification($business_id, $notification_type, $transaction, $contact)
    {
        ...
        if (! empty($notification_template)) {
          // dd($notification_template->auto_send);
    ...
    public function autoSendWelcomeNotification($business_id, $notification_type, $contact)
    {
        $notification_template = NotificationTemplate::where('business_id', $business_id)
                ->where('template_for', $notification_type)
                ->first();
        ...
        if (! empty($notification_template)) {
            if (! empty($notification_template->auto_send) || ! empty($notification_template->auto_send_sms) || ! empty($notification_template->auto_send_wa_notif)) {
                ...
                $tag_replaced_data = $this->replaceMsgTags($business_id, $orig_data, $contact);
                $data['email_body'] = $tag_replaced_data['email_body'];
                $data['sms_body'] = $tag_replaced_data['sms_body'];
                $data['whatsapp_text'] = $tag_replaced_data['whatsapp_text'];
                ...
                if (! empty($notification_template->auto_send_sms)) {
                    $data['mobile_number'] = $contact->mobile;
                    if(empty($contact->mobile)){
                        $data['mobile_number'] = $contact->contact_number;
                    }
                    // print_r($data);
                    // exit;
    ...
    public function replaceMsgTags($business_id, $org_data, $contact)
    {
        $business = Business::findOrFail($business_id);
        foreach ($org_data as $key => $value) {
            //Replace contact name
            if (strpos($value, '{name}') !== false) {
                $contact_name = $contact->surname.' '.$contact->first_name.' '.$contact->last_name;

                $data[$key] = str_replace('{name}', $contact_name, $org_data[$key]);
            }

            //Replace table
            if (strpos($value, '{user_name}') !== false) {

                $data[$key] = str_replace('{user_name}', $contact->username, $data[$key]);
            }
            if (strpos($value, '{password}') !== false) {

                $data[$key] = str_replace('{password}', $contact->password, $data[$key]);
            }
            if (strpos($value, '{login_url}') !== false) {
                // $login_url = '<a href="'.url('/login').'">Click Here</a>';
                $login_url = url('/login');
                $data[$key] = str_replace('{login_url}', $login_url,$data[$key]);
            }
            // dd($data);
            //Replace business_logo
            if (strpos($value, '{business_logo}') !== false) {
                $logo_name = $business->logo;
                $business_logo = ! empty($logo_name) ? '<img src="'.url('uploads/business_logos/'.$logo_name).'" alt="Business Logo" style="height:100px;width:100px;">' : '';
                $data[$key] = str_replace('{business_logo}', $business_logo, $data[$key]);
            }
        }
