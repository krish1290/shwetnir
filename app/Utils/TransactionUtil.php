...
class TransactionUtil extends Util
{
    ...
    public function createOrUpdatePaymentLinesDuplicate($transaction, $payments, $business_id = null, $user_id = null, $uf_data = true)
    {
        $payments_formatted = [];
        $edit_ids = [0];
        $account_transactions = [];
    ...
    public function getExpenseReport(
        ...
        if (!empty($filters['category'])) {
            $query->where(function ($query) use ($filters) {
                ...
                $query->orWhere('esc.id', $filters['category']);
            });
        }
    ...
    public function sumGroupTaxDetailsWithPercantage($group_tax_details)
    {
        $output = [];
        ...
        foreach ($group_tax_details as $group_tax_detail) {
            ...
            $output[$group_tax_detail['name']] .= number_format($group_tax_detail['calculated_tax'], 2) . " (" . $group_tax_detail['amount'] . ")%";
        }
    ...
    public function getProfitLossDetails($business_id, $location_id, $start_date, $end_date, $user_id = null)
    {
        ...
        $data['total_purchase_tax'] = !empty($purchase_details['total_purchase_inc_tax']) && !empty($purchase_details['total_purchase_exc_tax']) ? $purchase_details['total_purchase_inc_tax'] - $purchase_details['total_purchase_exc_tax'] : 0;
        $data['total_purchase_discount'] = !empty($total_purchase_discount) ? $total_purchase_discount : 0;
        $data['total_purchase_return'] = $transaction_totals['total_purchase_return_inc_tax'];

        //Sales
        $data['total_sell'] = !empty($sell_details['total_sell_exc_tax']) ? $sell_details['total_sell_exc_tax'] : 0;
        $data['total_sell_tax'] = !empty($sell_details['total_sell_exc_tax']) && !empty($sell_details['total_sell_inc_tax']) ? $sell_details['total_sell_inc_tax'] - $sell_details['total_sell_exc_tax'] : 0;
    ...
    public function addSellReturn($input, $business_id, $user_id, $uf_number = true)
    {
        ...
        foreach ($input['products'] as $product) {
            foreach ($sell->sell_lines as $sell_line) {
                if ($sell_line->id == $product['sell_line_id']) {
                    $trans_sell_line = TransactionSellLine::create(
                        [
    ...
    public function saveSellReturn($input, $business_id, $user_id, $uf_number = true)
    {
        $discount = [
            'discount_type' => $input['discount_type'] ?? 'fixed',
            'discount_amount' => $input['discount_amount'] ?? 0,
        ...
        $sell = Transaction::where('business_id', $business_id)
            ->with(['sell_lines', 'sell_lines.sub_unit'])
            ->findOrFail($input['transaction_id']);
        $old_sell_return_transaction_id = null;
        foreach ($input['products'] as $product) {
            if (!empty($product['sell_line_return_id']) && !empty($sell)) {
                //It means we are editing and we have primary key sell_line_return_id
                $actual_transaction = Transaction::where('business_id', $business_id)
                    ->with(['sell_lines', 'sell_lines.sub_unit'])
                    ->where('id', $sell->return_parent_id)
                    ->first();
                $old_sell_return_transaction_id = $sell->id;
                $sell = $actual_transaction;
                break;
            }
        }
        ...
        $sell_return_data = [
            ...
            'total_before_tax' => $invoice_total['total_before_tax'],
            'final_total' => $invoice_total['final_total'],
            'additional_notes' => $input['additional_note'] ?? null,
        ];
        ...
        if (empty($old_sell_return_transaction_id)) {
            $sell_return_data['transaction_date'] = $sell_return_data['transaction_date'] ?? \Carbon::now();
            $sell_return_data['business_id'] = $business_id;
            $sell_return_data['location_id'] = $sell->location_id;
            $sell_return_data['contact_id'] = $sell->contact_id;
            ...
            Transaction::where('id', $old_sell_return_transaction_id)->update($sell_return_data);
            $sell_return = Transaction::find($old_sell_return_transaction_id);
        }
            ...
            foreach ($sell->sell_lines as $sell_line) {
                if ($sell_line->id == $product['sell_line_id']) {
                    if ($input['discount_type'] == 'percentage') {
                        $discount = ($input['discount_amount'] / 100) * $product['unit_price'];
                    } else {
                    ...
                    $temp = [
                        ...
                        'unit_price_inc_tax' => $product['unit_price_inc_tax'],
                        'transaction_id' => $old_sell_return_transaction_id ?? $sell_return->id,
                        'parent_sell_line_id' => $sell_line->id,
                    ];

                    if (!empty($product['sell_line_return_id'])) {
                        $trans_sell_line = TransactionSellLine::where('id', $product['sell_line_return_id'])->update($temp);
                    } else {
                        $trans_sell_line = TransactionSellLine::create($temp);
                    }
                }
            }
        }
    ...
    public function savePurchaseReturn($input, $business_id, $user_id, $uf_number = true)
    {
        $discount = [
            'discount_type' => $input['discount_type'] ?? 'fixed',
            'discount_amount' => $input['discount_amount'] ?? 0,
        ...
        if (empty($sell_return_data['invoice_no']) && empty($sell_return)) {
            //Update reference count
            $ref_count = $this->setAndGetReferenceCount('purchase_return', $business_id);
            $sell_return_data['invoice_no'] = $this->generateReferenceNumber('purchase_return', $ref_count, $business_id);
        }
        ...
        if (empty($old_sell_return_transaction_id)) {
            ...
            $sell_return_data['contact_id'] = $sell->contact_id;
            $sell_return_data['customer_group_id'] = $sell->customer_group_id;
            $sell_return_data['type'] = 'purchase_return';
            $sell_return_data['status'] = 'final';
            $sell_return_data['created_by'] = $user_id;
            $sell_return_data['return_parent_id'] = $sell->id;
        ...
        foreach ($input['products'] as $product) {
            foreach ($sell->purchase_lines as $sell_line) {
                if ($sell_line->id == $product['sell_line_id']) {
                    if ($input['discount_type'] == 'percentage') {
                        $discount = ($input['discount_amount'] / 100) * $product['unit_price'];
                    } else {
                    ...
                    $temp = [
                        ...
                        'quantity' => $product['quantity'],
                        'quantity_returned' => $product['quantity'],
                        'pp_without_discount' => $product['unit_price'],
                        'purchase_price' => $product['unit_price'] - $discount,
                        'purchase_price_inc_tax' => $product['unit_price'] ?? 0,
                        'transaction_id' => $old_sell_return_transaction_id ?? $sell_return->id,
                        'parent_sell_line_id' => $sell_line->id,
                    ];

                    if (!empty($product['sell_line_return_id'])) {
                        $trans_sell_line = PurchaseLine::where('id', $product['sell_line_return_id'])->update($temp);
                    } else {
                        $trans_sell_line = PurchaseLine::create($temp);
                    }
                }
            }
        }
