...
class MfgUtil extends Util
{
    ...
    public function decreaseIngredientQuantity($variation_id)
    {
        $query = VariationLocationDetails::where('variation_id',$variation_id)->first();
        $avl_qty = $query->qty_available;
        if ($avl_qty > 0) {
           $new_qty = $avl_qty - 1;
           $query->update(['qty_available'=>$new_qty]);
        }else {
            $new_qty = 0;
            $query->update(['qty_available'=>$new_qty]);
        }
