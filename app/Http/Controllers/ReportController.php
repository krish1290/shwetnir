...
class ReportController extends Controller
{
     ...
     * @return void
     */
    public function __construct(TransactionUtil $transactionUtil, ProductUtil $productUtil, ModuleUtil $moduleUtil, BusinessUtil $businessUtil, EssentialsUtil $essentialsUtil)
    {
...
//
        ...
        if ($request->ajax()) {
            $contacts = Contact::where('contacts.business_id', $business_id)
                ...
                ->select(
                    ...
                    DB::raw("SUM(IF(t.type = 'purchase', tp.amount, 0)) as total_payment_to_vendor"),
                    DB::raw("SUM(IF(t.type = 'sell', tp.amount, 0)) as total_payment_from_customer")
            ...
            return Datatables::of($contacts)
                ...
                ->editColumn(
                    ...
                    '<span class="total_payment_to_vendor" data-orig-value="{{$total_payment_to_vendor}}">@format_currency($total_payment_to_vendor)</span>'
                )
                ->editColumn(
                    'total_payment_from_customer',
                    '<span class="total_payment_from_customer" data-orig-value="{{$total_payment_from_customer}}">@format_currency($total_payment_from_customer)</span>'
                )
                ...
                ->rawColumns(['total_purchase', 'total_invoice', 'due', 'name', 'total_purchase_return', 'total_sell_return', 'opening_balance_due', 'total_payment_from_customer', 'total_payment_to_vendor'])
                ->make(true);
        }
            ...
            // })
                ...
                ->addColumn('edit', function ($row) {
                    ...
                    if (!empty($row->exp_date)) {
                        ...
                        $carbon_now = \Carbon::now();
                        if ($carbon_now->diffInDays($carbon_exp, false) < 0) {
                            $html .= ' <button type="button" class="btn btn-warning btn-xs remove_from_stock_btn" data-href="' . action([\App\Http\Controllers\WastageController::class, 'removeExpiredStock'], [$row->purchase_line_id]) . '"> <i class="fa fa-trash"></i> ' . __('lang_v1.remove_from_stock') .
                                '</button>';
                        }
                    }
    ...
    public function getproductPurchaseReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = PurchaseLine::join(
                ...
                ->leftjoin('tax_rates', 'purchase_lines.tax_id', '=', 'tax_rates.id')
                ->leftjoin('units as u', 'p.unit_id', '=', 'u.id')
                ->where('t.business_id', $business_id)
            ...
            return Datatables::of($query)
                ...
                ->rawColumns(['ref_no', 'unit_purchase_price', 'subtotal', 'purchase_qty', 'quantity_adjusted', 'tax', 'supplier'])
                ->make(true);
        }
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionPayment::leftjoin('transactions as t', function ($join) use ($business_id) {
                ...
                ->select(
                    ...
                    DB::raw("(select sum(t1.final_total) FROM transactions as t1 WHERE t1.return_parent_id = t.id ) as payment_from_vendor"),
            ...
            return Datatables::of($query)
                ...
                ->editColumn('payment_from_vendor', function ($row) {
                    return '<span class="paid-amount" data-orig-value="' . $row->payment_from_vendor . '">' .
                    $this->transactionUtil->num_f($row->payment_from_vendor, true) . '</span>';
                })
                ...
                ->rawColumns(['ref_no', 'amount', 'method', 'action', 'supplier', 'payment_from_vendor'])
                ->make(true);
        }
        $business_locations = BusinessLocation::forDropdown($business_id);
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionPayment::leftjoin('transactions as t', function ($join) use ($business_id) {
                ...
                ->select(
                    ...
                    DB::raw("(select sum(t1.final_total) FROM transactions as t1 WHERE t1.return_parent_id = t.id  ) as payment_to_customer"),
                    'transaction_payments.amount',
            ...
            return Datatables::of($query)
                ...
                ->editColumn('payment_to_customer', function ($row) {
                    $amount = $row->payment_to_customer;
                ...
                ->rawColumns(['invoice_no', 'amount', 'method', 'action', 'customer', 'payment_to_customer'])
                ->make(true);
        }
    ...
    public function claimReimbursementReport(Request $request)
    {
        ...
        $status = ['' => 'Select Status','2' => __('Approved'),'1' => __('Unapproved')];
        $business_locations = BusinessLocation::forDropdown($business_id, false);
        ...
        return view('report.claim_reimbursement_report')
            ->with(compact('business_locations', 'taxes', 'tax_report_tabs', 'contact_dropdown','status'));
    }

    public function getClaimReimburseMentDetails(Request $request)
    {
      if (!auth()->user()->can('claim_reimbursement.view')) {
          abort(403, 'Unauthorized action.');
      }
      ...
      $this->claim_statuses = [null => ['class' => 'badge-warning', 'label' => 'Pending', 'style' => ''],
          ...
          1 => ['class' => 'badge-danger', 'label' => 'UnApproved', 'style' => ''],
          2 => ['class' => 'badge-success', 'label' => 'Approved', 'style' => ''],
      ];
      ...
      if ($request->ajax()) {
          ...
          $data = EssentialsClaimReimbursement::where('business_id', $business_id)->whereIn('id', $essential_ids)
              ->with('employees');
          if (auth()->user()->can('essentials.approve_claim_reimbursement') || auth()->user()->can('superadmin')) {
              $data = EssentialsClaimReimbursement::where('business_id', $business_id)
                  ->with('employees');
          }
              if (!empty($status)) {
                  $data->where('is_approved', $status);
          }
          if (!empty($start_date) && !empty($end_date)) {
          ...
          return $datatable->rawColumns(['amount', 'is_approved'])
              ->make(true);
    }
  }
}
