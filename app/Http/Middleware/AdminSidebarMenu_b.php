...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            //Home
            $menu->url(action([\App\Http\Controllers\HomeController::class, 'index']), __('home.home'), ['icon' => 'fas fa-home', 'has_sub_item' => 'no', 'active' => request()->segment(1) == 'home'])->order(5);
            ...
            if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping', 'access_sell_return', 'direct_sell.view', 'direct_sell.update', 'access_own_sell_return'])) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
                        if (!empty($pos_settings['enable_sales_order']) && ($is_admin || auth()->user()->hasAnyPermission(['so.view_own', 'so.view_all', 'so.create']))) {
                            $sub->url(
                                ...
                                ['icon' => 'fas fa-arrow-down', 'active' => request()->segment(1) == 'sales-order']
                            );
                        }
                    ...
                    ['icon' => 'fas fa-arrow-circle-up', 'id' => 'tour_step7']
                )->order(30);
            }
                    ...
                    function ($sub) {
                        if (auth()->user()->can('purchase.view')) {
                            $sub->url(
                                ...
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'wastage-management' && request()->segment(2) == null]
                            );
                        }
                        if (auth()->user()->can('purchase.create')) {
                            $sub->url(
                                ...
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'wastage-management' && request()->segment(2) == 'create']
                            );
                        }
                    },
